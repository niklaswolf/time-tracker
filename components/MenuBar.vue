...
<template>
  <div
    ...
    <div class="bg-light-800 @dark:bg-dark-600 text-neutral-800 @dark:text-neutral-300 flex flex-col grow border-rd relative">
      <div class="p-2">
        <a href="#" class="flex items-center text-neutral-800 @dark:text-neutral-300 no-underline hover:underline">
          ...
          <div class="ml-2 ">ABC-123 - Ticket name</div>
        </a>
      </div>
      <div class="grow flex items-center bg-light-100 @dark:bg-dark-400 p-2">
        ...
        <template v-if="!isRunning">
          ...
          <button v-else @click="startTimer" class="rounded-full border-none bg-neutral-300 @dark:bg-neutral-400 cursor-pointer flex items-center">
            <span class="i-mdi-play"></span>
            <span>Start</span>
          </button>
        </template>
        <template v-else>
          <button @click="pauseTimer" class="rounded-full border-none bg-neutral-300 @dark:bg-neutral-400 cursor-pointer flex items-center">
            <span class="i-mdi-pause"></span>
            <span>Pause</span>
          </button>
          <button @click="stopTimer()" class="rounded-full border-none bg-neutral-300 @dark:bg-neutral-400 cursor-pointer flex items-center">
      ...
      <button @click="handleOpenMain" class="absolute bottom-0 right-0 rounded-full border-none bg-transparent cursor-pointer flex items-center">
